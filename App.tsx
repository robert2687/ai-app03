...
const App: React.FC = () =>
{
  // Initialize with env var if available
  ...
  useEffect( () =>
  {
    const runNextAgent = async () =>
    {
      if ( isWorkflowRunning && !isProcessing && completedAgentCount < agents.length )
      {
        ...
        try
        {
          ...
          const outputContent = await runAgent( currentAgent, context, apiKey );
    ...
    runNextAgent();
  }, [ isWorkflowRunning, isProcessing, completedAgentCount, agents, apiKey ] );
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-gray-900 text-white">
        <div className="text-center space-y-4 max-w-md px-6">
          ...
          <p className="text-gray-400">Please provide your Gemini API Key to continue.</p>
          <input
            type="password"
          ...
          <button
            ...
            className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-50 px-6 py-3 rounded-lg font-semibold transition-colors"
          >
            Connect API Key
          </button>
          <div className="mt-4">
            <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" rel="noopener noreferrer" className="text-sm text-blue-400 hover:underline">
              Get an API Key
            </a>
          </div>
        </div>
